...
func TestDefaultClassifier(t *testing.T) {
	cases := []struct {
		...
		{"abort", txnpure.KindRollback},
		{"RoLlBaCk", txnpure.KindRollback},
		{"ROLLBACK TOO", txnpure.KindRollback}, // TOO is not the TO keyword
		...
		{"alter table t add c int", txnpure.KindWrite},
		{"grant select on t to u", txnpure.KindWrite},
		{"revoke select on t from u", txnpure.KindWrite},
		{"comment on table t is 'x'", txnpure.KindWrite},
		{"refresh materialized view v", txnpure.KindWrite},
		{"call p()", txnpure.KindWrite},
		{"do $$ select 1 $$", txnpure.KindWrite},
		{"copy t from stdin", txnpure.KindWrite},
		{"import into t csv data ('f')", txnpure.KindWrite},
		{"upsert into t values (1)", txnpure.KindWrite},
		{"replace into t values (1)", txnpure.KindWrite},
		...
		{"wItH x aS (uPdAtE t SET a = 1 RETURNING id) SELECT * FROM x", txnpure.KindWrite},
		{"", txnpure.KindOther},
